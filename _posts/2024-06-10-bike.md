...
* Using the sensor as an input for video games (this post)
...
For some games (mostly on the PS4) I just have an alarm for if my cadence drops below a set threshold. That sort of thing is dealt with at the speedometer page. This page is just for what to do with the keypresses after they are triggered by the speedometer. 

However, I also have some code that converts the input from [my custom speedometer](https://joereddington.com/2025/03/09/advanced-spin-bike-speedometer.html) into keypresses, which I use to play retro computer games. The code is in the [ViciousCycle repo](https://github.com/joereddington/viciouscycle).



# History

## Version 1: Bluetooth sensor and browser game with Python on laptop
...
## Version 2: actual game, still with Python on the laptop
...
While doing it, I rebuilt the code in general, and investigated what might be possible with the device on other games (it won't be needed for a while, Road Rash is wearing me out at the moment). Presumably the setup of 'hold down the accelerate button' will work for similar retro games like Mario Kart.
...
That eliminates the 'direct control' option immediately. 0.7 seconds plus the sensor lag (it will take a while before it can tell I've sped up) is far too long for any sort of gaming response. There are other problems as well but I talk through those in the other post.
...
## Version 3: the speedometer now produces the key presses that are used to control the game
